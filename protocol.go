...
// EventType describes a named event data structure type.
...
type Protocol struct {
	...
	gameEventTypes       []EventType // Game event type descriptors; index is event id
	gameEventIdTypeId    int         // The typeid of the NNet.Game.EEventId enum
	messageEventTypes    []EventType // Message event type descriptors; index is event id
	messageEventIdTypeId int         // The typeid of the NNet.Game.EMessageId enum
	trackerEventTypes    []EventType // Tracker event type descriptors; index is event id
...
func parseProtocol(src string, baseBuild int) *Protocol {
	...
	parseEventTypes := func(stripPref, stripPost string) []EventType {
		var err error
		em := make(map[int]EventType) // First build it in a map
	...
	p.gameEventTypes = parseEventTypes("NNet.Game.S", "Event")
	...
	p.messageEventTypes = parseEventTypes("NNet.Game.S", "Message")
	...
	if p.hasTrackerEvents {
		...
		p.trackerEventTypes = parseEventTypes("NNet.Replay.Tracker.S", "Event")
...
// DecodeAttributesEvents decodes and returns the attributes events.
// Panics if decoding fails.
func (p *Protocol) DecodeAttributesEvents(contents []byte) Struct {
	s := Struct{}
...
// DecodeGameEvents decodes and returns the game events.
// In case of a decoding error, successfully decoded events are still returned along with an error.
func (p *Protocol) DecodeGameEvents(contents []byte) ([]Event, error) {
	return p.decodeEvents(newBitPackedDec(contents, p.typeInfos), p.gameEventIdTypeId, p.gameEventTypes, true)
}

// DecodeMessageEvents decodes and returns the message events.
// In case of a decoding error, successfully decoded events are still returned along with an error.
func (p *Protocol) DecodeMessageEvents(contents []byte) ([]Event, error) {
	return p.decodeEvents(newBitPackedDec(contents, p.typeInfos), p.messageEventIdTypeId, p.messageEventTypes, true)
}

// DecodeTrackerEvents decodes and returns the tracker events.
// In case of a decoding error, successfully decoded events are still returned along with an error.
func (p *Protocol) DecodeTrackerEvents(contents []byte) ([]Event, error) {
	return p.decodeEvents(newVersionedDec(contents, p.typeInfos), p.trackerEventIdTypeId, p.trackerEventTypes, false)
}

// decodeEvents decodes a series of events.
// In case of a decoding error, successfully decoded events are still returned along with an error.
func (p *Protocol) decodeEvents(d decoder, evtidTypeid int, etypes []EventType, decUserID bool) (events []Event, err error) {
